...
Reviewed against the **26-build / 171-profile release `8ce872cfd`** on
22 September 2026. This is a list of remaining DPS uncertainties, not a request
to retest every datamined number. No engine rules or rankings changed in this
...
**Penance timing:** the current tooltip explicitly says an immediate bolt,
then one each second for two seconds. The engine instead emits at approximately
0.667, 1.333 and 2 seconds. This is an identified implementation discrepancy;
another recording is not needed to establish the stated schedule. Haste remains
a separate question.
...
| **T02** | Level-60/off-hand rage. Existing low-level logs support damage-independent generation, but do not establish a complete Fury formula. This can change every Warrior comparison. | Supply weapon speed, level and matching resource/equipment logs for one- and two-handed weapons and the off-hand. Isolate outgoing attacks from spending, incoming damage and decay. Level-20 off-hand data helps; high-level scaling still needs later access. [Existing evidence](check_dispositions.md#rage-evidence). |
...
| **T15 / T16 / T35** | Haste-to-Energy scaling and avoided-attack refunds. Base recovery is sourced at 10/sec; smooth recovery is already reported and modeled. General-haste scaling, 80% builder refunds and zero finisher refunds remain assumptions. | At level 20, compare usable Energy recovery with one named haste effect; do not treat attack-speed-only Slice and Dice as general haste. Separately record paid cost and miss/dodge refunds, subtracting natural regeneration. Gnomes should include a discounted cast. |
...
| **T21** | Maelstrom's actual proc rate and its item/free-cast interactions. The current 2 PPM per talent point is not established by the captured client. | Maelstrom talent: compare two weapon speeds and isolate Windfury extras; check Clearcasting around five-stack LB. Totem of the Storm needs level-60 access. Maelstrom applies to LB, not Chain Lightning. |
...
- **T37:** Death's unexplained script value 150 could affect damage; retain
  ...
  backlash are implemented. Absorb/healer-survival tests are not part of this
  DPS encounter.
