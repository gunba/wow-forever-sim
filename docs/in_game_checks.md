...
| **T57** | Seal of Righteousness adds its spell-power coefficient twice in the inherited implementation; its additional two-hand multiplier cites SoD testing, not Forever evidence. This affects tank damage and seal-using alternatives. | At level 20, keep rank and weapon fixed, compare noncritical seal procs at two known SP totals, then repeat with a two-hander. Record talents and exclude weapon-proc damage. The slope, not just a tooltip's base damage, distinguishes one versus two coefficient applications. |
